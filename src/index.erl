...
%% events generated by selecting items from the menubar
...
event(grid) ->
    wf:update(west, #panel{id = west, body = [grid:control_panel(jqgrid_example_tag)]}),
    wf:update(center, #panel{id = center, body = [grid:body(jqgrid_example_tag)]}),
...
event({jqgrid_example_tag, Event}) ->
    grid:event({jqgrid_example_tag, Event});
